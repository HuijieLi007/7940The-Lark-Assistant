(function(win, options, undefined){
	...
	var _generateHtml = function(source){
		...
		html.push('<a href="javascript:void(0);" style="display:none" class="'+ cssfix +'icon-max'+ (_con.ismax ? ' '+ cssfix +'icon-nor' : '') +'"><em class="'+ cssfix +'icon"></em></a>');

		html.push('<a href="javascript:void(0);" style="display:none" class="'+ cssfix +'icon-clo"><em class="'+ cssfix +'icon"></em></a>');
		html.push('</div>');
		html.push('</div>');
		//content
		...
		if(!!_con.showCopyright){
			html.push('<a class="'+ cssfix +'clink" href="http://smartnlp.cn" target="_blank">Icon powered by iconfinder</a>');
		};
		html.push('<a class="'+ cssfix +'btn-send" style="background-color:'+ _colors.button +'">send</a>');
		html.push('</div>');
		html.push('</div>');
		html.push('</div>');
		...
		if(_con.showLeftPanel){
			...
			html.push('<a href="javascript:void(0);" data-index="0" class="'+ cssfix +'on">questions</a>');
			...
			for(var i=0,list = _con.quicks,len = list.length; i<len; i++){
				html.push('<li><a href="javascript:void(0);">'+ list[i][0] +'</a></li>');
			};
			html.push('</ul>');
	...
	Robot.prototype = {
		...
		, 'init': function(){
			...
			// document.getElementsByClassName('smartnlp-robot-slide-btn')[0].click();
			// document.getElementsByClassName('smartnlp-robot-icon-clo')[0].click();
			// document.getElementsByClassName('smartnlp-robot-icon-max')[0].click();
		}
		//显示欢迎词
		, 'welcome': function(){
			var _this = this
		...
		, 'send': function(text){
			...
			_jsonp({
				...
				, 'error': function(){
					var err = 'cannot handle this keyword,<br> maybe you can try these keywords:<br>';
					var response = '';
					...
					if(response != '') {
						...
						$('.'+ cssfix +'msg-mm .'+ cssfix +'htmcont', $answer).html(err);
					}
				}
			});
		}
